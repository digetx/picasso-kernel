...
static void ec_reboot(enum reboot_mode reboot_mode, const char *cmd)
{
	...
	if (!ec_chip->is_panic)
		...
		if (ec_chip->arm_reboot)
			ec_chip->arm_reboot(reboot_mode, cmd);
		else
...
static int ec_remove(struct i2c_client *client)
{
	...
	if (arm_pm_restart == ec_reboot)
		arm_pm_restart = ec_chip->arm_reboot;

	if (pm_power_off == ec_poweroff)
		pm_power_off = NULL;
