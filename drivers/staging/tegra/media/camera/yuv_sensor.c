/*
 * kernel/drivers/media/platform/tegra
 *
...
static void poll_current_state(u16 expected_state)
{
	...
	} while (state != expected_state && --retry);
...
static int sensor_set_mode(struct sensor_info *info, struct sensor_mode *mode)
{
	...
	if (!(val == SENSOR_800_WIDTH_VAL || val == SENSOR_720_WIDTH_VAL || val == SENSOR_1600_WIDTH_VAL)
									&& sensor_table == SENSOR_MODE_1600x1200) {
...
static int yuv_initialize(struct sensor_info *info){
	...
	tegra_camera_clk_set_rate(&clk_info);
...
static struct of_device_id yuv_sensor_of_match[] = {
	{ .compatible = "aptina,mt9d115", },
	{ },
};
MODULE_DEVICE_TABLE(of, yuv_sensor_of_match);

static struct i2c_driver yuv_sensor_driver = {
	.driver = {
		.name = SENSOR_NAME,
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(yuv_sensor_of_match),
	},
	.probe = sensor_probe,
	.remove = sensor_remove,
	.id_table = sensor_id,
};
module_i2c_driver(yuv_sensor_driver);
