...
static struct ov5650_reg mode_start[] = {
	...
	{0x401d, 0x08}, /* BLC control pg 120 */
	...
	{0x3011, 0x14}, /* PLL control02 R_DIVP [5:0] div 24 pg 99 */
...
static struct ov5650_reg mode_2080x1164[] = {
	...
	{0x3010, 0x10}, // PLL control01 DIVM [3:0] DIVS [7:4] div 1 pg 99
	{0x3011, 0x14}, // PLL control02 R_DIVP [5:0] div 24 pg 99 (20Mhz Mclk)
	{0x3012, 0x02}, // PLL CTR 03, default
...
static int ov5650_set_power(int val)
{
	...
	case StereoCameraMode_Stereo:
		if (stereo_ov5650_info->left.pdata) {
			if (val && stereo_ov5650_info->left.pdata->power_on)
				stereo_ov5650_info->left.pdata->power_on();
			else if (stereo_ov5650_info->left.pdata->power_off)
				stereo_ov5650_info->left.pdata->power_off();
		}
		if (stereo_ov5650_info->right.pdata) {
			if (val && stereo_ov5650_info->right.pdata->power_on)
				stereo_ov5650_info->right.pdata->power_on();
			else if (stereo_ov5650_info->right.pdata->power_off)
				stereo_ov5650_info->right.pdata->power_off();
		}
		break;
			...
			const struct i2c_device_id *id)
{
	...
	if (!tegra_camera_probed())
		return -EPROBE_DEFER;

	if (!stereo_ov5650_info) {
		stereo_ov5650_info = devm_kzalloc(&client->dev,
						  sizeof(*stereo_ov5650_info),
						  GFP_KERNEL);
		if (!stereo_ov5650_info) {
		...
		s = &stereo_ov5650_info->left;

		s->pwdn_gpio = of_get_named_gpio(np, "pwdn-gpio", 0);
		if (s->pwdn_gpio == -EPROBE_DEFER)
			return -EPROBE_DEFER;

		s->rst_gpio = of_get_named_gpio(np, "rst-gpio", 0);
...
static struct of_device_id left_ov5650_sensor_of_match[] = {
	{ .compatible = "omnivision,ov5650l", },
	{ },
};
MODULE_DEVICE_TABLE(of, left_ov5650_sensor_of_match);

static struct i2c_driver left_ov5650_i2c_driver = {
	.driver = {
		.name = "ov5650",
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(left_ov5650_sensor_of_match),
	},
	.probe = left_ov5650_probe,
	.remove = left_ov5650_remove,
	.id_table = left_ov5650_id,
};
module_i2c_driver(left_ov5650_i2c_driver);
...
static struct of_device_id right_ov5650_sensor_of_match[] = {
	{ .compatible = "omnivision,ov5650r", },
	{ },
};
MODULE_DEVICE_TABLE(of, right_ov5650_sensor_of_match);

static struct i2c_driver right_ov5650_i2c_driver = {
	.driver = {
		.name = "ov5650R",
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(right_ov5650_sensor_of_match),
	},
	.probe = right_ov5650_probe,
	.remove = right_ov5650_remove,
	.id_table = right_ov5650_id,
};
module_i2c_driver(right_ov5650_i2c_driver);
