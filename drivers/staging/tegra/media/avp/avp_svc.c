				...
				size_t len)
{
	...
	if (mod->clk_req == CLK_REQUEST_AVP) {
				  ...
				  struct trpc_node *rpc_node)
{
	...
	for (i = 0; i < NUM_AVP_MODULES; i++) {
		...
		if (mod->clk_req != CLK_REQUEST_AVP) {
			...
			if (IS_ERR(clk)) {
				ret = PTR_ERR(clk);
				pr_err("avp_svc: Couldn't get %s clock\n", mod->name);
				goto err_get_clks;
			}
			rst = devm_reset_control_get(&pdev->dev, mod->name);
			if (IS_ERR(rst)) {
				pr_err("avp_svc: Failed to get %s reset\n", mod->name);
				ret = PTR_ERR(rst);
				goto err_get_clks;
			}
		}
		avp_svc->clks[mod->clk_req].clk = clk;
		avp_svc->clks[mod->clk_req].rst = rst;
