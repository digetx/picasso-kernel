...
static int tegra_avp_probe(struct platform_device *pdev)
{
	...
	avp->recv_wq = alloc_workqueue("avp-msg-recv", WQ_HIGHPRI, 1);
	if (!avp->recv_wq) {
...
static struct of_device_id tegra_avp_of_match[] = {
	{ .compatible = "nvidia,tegra20-avp", },
	{ },
};
MODULE_DEVICE_TABLE(of, tegra_avp_of_match);
...
static struct platform_driver tegra_avp_driver = {
	...
	.driver = {
		.name	= TEGRA_AVP_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = of_match_ptr(tegra_avp_of_match),
	},
};
module_platform_driver(tegra_avp_driver);
