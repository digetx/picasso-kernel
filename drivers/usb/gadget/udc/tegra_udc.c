...
#define IO_AHB_GIZMO_PHYS	0x6000c004
#define IO_AHB_GIZMO_SIZE	0x10c
...
static void tegra_usb_phy_memory_prefetch_on(struct tegra_udc *udc)
{
	unsigned long val;

	val = readl(udc->ahb_gizmo + AHB_MEM_PREFETCH_CFG1);
	val |= PREFETCH_ENB;
	writel(val, udc->ahb_gizmo + AHB_MEM_PREFETCH_CFG1);
	val = readl(udc->ahb_gizmo + AHB_MEM_PREFETCH_CFG2);
	val |= PREFETCH_ENB;
	writel(val, udc->ahb_gizmo + AHB_MEM_PREFETCH_CFG2);
}

static void tegra_usb_phy_memory_prefetch_off(struct tegra_udc *udc)
{
	unsigned long val;
...
static void dr_controller_run(struct tegra_udc *udc)
{
	...
	if (!udc->transceiver) {
		...
		temp |= (USB_SYS_VBUS_WAKEUP_INT_ENABLE | USB_INTR_RESET_EN
...
static void tegra_queue_td(struct tegra_ep *ep, struct tegra_req *req)
{
	...
	tegra_usb_phy_memory_prefetch_on(udc);
			    ...
			    gfp_t gfp_flags)
{
	...
	tegra_usb_phy_memory_prefetch_off(udc);
		...
		struct tegra_req *curr_req)
{
	...
	for (j = 0; j < curr_req->dtd_count; j++) {
		...
		readb(udc->to_io + USB1_PREFETCH_ID);
...
static int tegra_udc_probe(struct platform_device *pdev)
{
	...
	if (!udc->irq) {
		err = -ENODEV;
		dev_err(&pdev->dev, "failed to get platform irq resources\n");
		return err;
	}

	err = devm_request_irq(&pdev->dev, udc->irq, tegra_udc_irq,
	...
	err = usb_phy_init(udc->phy);
	if (err) {
		dev_err(&pdev->dev, "failed to init USB phy\n");
		goto err_phy;
	}
	...
	udc->to_io = devm_ioremap(&pdev->dev, IO_PPCS_PHYS, IO_PPCS_SIZE);
	if (!udc->to_io)
		goto err_del_udc;

	udc->ahb_gizmo = devm_ioremap(&pdev->dev,
				  IO_AHB_GIZMO_PHYS, IO_AHB_GIZMO_SIZE);
	if (!udc->ahb_gizmo)
		goto err_del_udc;
