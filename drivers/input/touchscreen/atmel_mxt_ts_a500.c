...
static int mxt_check_reg_init(struct mxt_data *data)
{
	...
	if (!pdata->config) {
		dev_info(dev, "No cfg data defined, using default\n");
		pdata->config = default_config;
		pdata->config_length = sizeof(default_config);
	}
	...
	for (i = 0; i < data->info.object_num; i++) {
		...
		if (!mxt_object_writable(object->type))
			continue;
		dev_info(dev, "Writing object type %d, config offset %d",
			 data->object_table[i].type, index);
		for (j = 0;
...
static void mxt_start(struct mxt_data *data)
{
	...
	error = mxt_write_object(data, MXT_TOUCH_MULTI_T9,
				 MXT_TOUCH_CTRL, MXT_TOUCH_ENABLE);
...
static void mxt_stop(struct mxt_data *data)
{
	...
	if (!error)
		...
		dev_err(dev, "MXT failed to suspend\n");
					      ...
					      struct device_node *of_node)
{
	...
	if (!of_property_read_u32(of_node, "x-size", &val))
		pdata->x_size = val;
	else
		pdata->x_size = 1280;

	if (!of_property_read_u32(of_node, "y-size", &val))
		pdata->y_size = val;
	else
		pdata->y_size = 800;
	...
	pdata->config_length = cells = length / sizeof(u8);

	config = devm_kzalloc(&client->dev, sizeof(*config) * cells, GFP_KERNEL);
	if (!config)
		return NULL;

	if (of_property_read_u8_array(of_node, "config", config, cells))
		return NULL;
	pdata->config = config;
		...
		const struct i2c_device_id *id)
{
	...
	data->mem_access_attr.attr.mode = S_IRUGO | S_IWUGO | S_IWGRP;
...
static struct i2c_driver mxt_driver = {
	.driver = {
		.name	= "atmel_mxt_ts_a500",
		.owner	= THIS_MODULE,
		.of_match_table	= mxt_dt_ids,
#if defined(CONFIG_PM_SLEEP) && !defined(CONFIG_PM_EARLYSUSPEND)
		.pm	= &mxt_pm_ops,
#endif
	},
	.probe		= mxt_probe,
