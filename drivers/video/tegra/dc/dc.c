...
void tegra_enable_backlight(struct tegra_dc *dc)
{
	...
	if (!IS_ERR_OR_NULL(dc->out->bl_vdd))
		WARN_ON(regulator_enable(dc->out->bl_vdd));
}
	...
	switch (dc->out->type) {
		case TEGRA_DC_OUT_RGB:
			if (!IS_ERR_OR_NULL(dc->out->pnl_vdd))
				WARN_ON(regulator_enable(dc->out->pnl_vdd));
			if (!IS_ERR_OR_NULL(dc->out->lvds_vdd))
				WARN_ON(regulator_enable(dc->out->lvds_vdd));
			break;
		case TEGRA_DC_OUT_HDMI:
			if (!IS_ERR_OR_NULL(dc->out->hdmi_vdd))
				WARN_ON(regulator_enable(dc->out->hdmi_vdd));
			if (!IS_ERR_OR_NULL(dc->out->hdmi_pll))
				WARN_ON(regulator_enable(dc->out->hdmi_pll));
			break;
		case TEGRA_DC_OUT_DSI:
			break;
		default:
