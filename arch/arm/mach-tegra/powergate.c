...
static struct clk* get_partition_clk(int domain_id, int clk_id) {
	struct powergate_partition *p = &powergate_partition_info[domain_id];
	struct partition_clk_info *ci = &p->clk_info[clk_id];

	if (!ci->clk_name)
		return ERR_PTR(-ENOENT);
	
	ci->clk_ptr = clk_get(NULL, ci->clk_name);
	if (IS_ERR(ci->clk_ptr))
		pr_err("Can't get powergate clk: %s\n", ci->clk_name);
